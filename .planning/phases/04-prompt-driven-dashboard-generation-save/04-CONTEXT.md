...
**Status:** Ready for planning — LLM provider question resolved (2026-08-12): using EdgeOne Makers' built-in AI Gateway (see D-04)
...
- **D-04:** [RESOLVED 2026-08-12, human decision] Provider: **EdgeOne Makers' built-in AI Gateway** (`https://ai-gateway.edgeone.link/v1/chat/completions`, OpenAI-compatible REST), model `@makers/deepseek-v4-flash`. This is EdgeOne Makers' own built-in/free model tier — no external vendor account or third-party data-sharing decision needed, and it fits this project's platform-native precedent (same console/CLI as everything else already built). The LLM call happens **server-side, from an Edge Function** (e.g. `edge-functions/api/generate.js`), following the exact `context.env` secret pattern established in Phase 1 (`PLACEHOLDER_OIDC_CLIENT_ID`) and Phase 2/3 (`SESSION_SIGNING_KEY`, OIDC secrets): a `MAKERS_MODELS_KEY` (generated via EdgeOne Makers console → Models → API Key) read from `env`, never sent to or readable by the client, never echoed in any response. The call uses plain `fetch()` against the gateway's REST endpoint — **not an npm SDK** — following Phase 3's precedent of avoiding an SDK dependency after `tencentcloud-sdk-nodejs` proved incompatible with the edge/V8-isolate runtime; the gateway is a plain HTTPS JSON REST API, so this is edge-safe by construction.
  - **Known caveat (from official EdgeOne Makers docs):** built-in models are explicitly scoped for prototyping/technical validation ("不保证模型输出效果质量，请勿在生产环境使用" — not guaranteed for production use, subject to automatic rate-limiting on abusive/non-organic usage patterns). Acceptable for this project's v1 scope; if production-grade reliability is needed later, the same gateway supports binding an external vendor key (OpenAI/Anthropic/Hunyuan/etc.) under the same OpenAI-compatible request shape — a one-line `model` string swap, no code restructuring needed.
  - **JSON-mode/structured-output support is unconfirmed** for the built-in tier (not documented) — Phase 4 research/planning must verify empirically whether `response_format: {type:"json_object"}` works, or fall back to prompt-engineering the JSON-only instruction plus D-03's strict server-side schema validation (which is already the safety backstop regardless of native JSON-mode support).
